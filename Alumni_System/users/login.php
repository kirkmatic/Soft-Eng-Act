...
?>
<body class="bg-gray-100 flex items-center justify-center h-screen" style="background-image: url('../assets/bg.jpg'); background-size: cover; background-position: center;"></body>
    <!-- Central Container -->
    <div class="w-full md:w-2/3 lg:w-1/2 bg-white rounded-xl shadow-lg relative flex flex-col md:flex-row items-center justify-center">
        ...
        <div class="w-full md:w-1/2 px-8 flex flex-col items-center justify-center animate__animated animate__fadeIn">
            ...
            <form class="flex flex-col w-full" action="../backend/login_handler.php" method="POST">
                ...
                <button type="submit" name="login_btn" class="w-full bg-gradient-to-r from-indigo-600 to-indigo-900 text-xl text-white p-4 rounded-xl hover:bg-grey-200 transition font-semibold mb-2">Login</button>
            </form>
            <p>Don't have an Account? <a class="font-semibold text-indigo-900" href="signup.php">Sign up </a></p>
        </div>

        <!-- Toggle Panel -->
        <div class="w-full md:w-1/2 h-full bg-gradient-to-b from-indigo-600 to-indigo-900 text-white flex flex-col items-center justify-center transition-transform duration-500 py-20 rounded-tr-xl rounded-br-xl">
            <img src="../assets/logos.png" alt="University Logo" class="w-25 h-auto mx-auto mb-4">
            <h1 class="text-2xl font-bold mb-2">Welcome</h1>
            <h1 class="text-xl">Alumni System</h1>
            <button class="mt-4 px-6 py-2 text-indigo-50 border-solid border-2 border-indigo-50 rounded-xl" onclick="window.location.href='signup.php'">
